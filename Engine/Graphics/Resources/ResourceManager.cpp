...
{
	using namespace std::string_literals;
	const std::string key = typeid(IndexBuffer).name() + "#"s + std::to_string(Indecies->size()) + "{" + std::to_string((*Indecies)[0]) + "#"s + std::to_string((*Indecies)[1]) + "#"s + std::to_string((*Indecies)[2]) + "}";
	const auto i = Bindables.find(key);
	if (i == Bindables.end())
	{
...
std::shared_ptr<RootSignature> ResourceManager::CreateRootSignature(Drawable* pDrawable, std::string& PSO_Key, RS_Layout& Lay) noexcept
{
	using namespace std::string_literals;
...
void ResourceManager::InitializeResourcesOfScene(Scene* pScene)
{
	// Initialize Heap.
