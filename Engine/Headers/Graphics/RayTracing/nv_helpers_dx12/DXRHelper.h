...
namespace nv_helpers_dx12
{
        ...
        const D3D12_HEAP_PROPERTIES& heapProps)
    {
        ...
        Error_Check(m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufDesc,
...
#define ROUND_UP(v, powerOf2Alignment) (((v) + (powerOf2Alignment)-1) & ~((powerOf2Alignment)-1))
    ...
    IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
    {
        ...
        if (!pCompiler)
        {
            Error_Check(DxcCreateInstance(CLSID_DxcCompiler, __uuidof(IDxcCompiler), (void**)&pCompiler));
            Error_Check(DxcCreateInstance(CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void**)&pLibrary));
            Error_Check(pLibrary->CreateIncludeHandler(&dxcIncludeHandler));
        }
        ...
        Error_Check(pLibrary->CreateBlobWithEncodingFromPinned(
            (LPBYTE)sShader.c_str(), (uint32_t)sShader.size(), 0, &pTextBlob));

        // Compile
        IDxcOperationResult* pResult;
        Error_Check(pCompiler->Compile(pTextBlob, fileName, L"", L"lib_6_3", nullptr, 0, nullptr, 0,
            dxcIncludeHandler, &pResult));

        // Verify the result
        HRESULT resultCode;
        Error_Check(pResult->GetStatus(&resultCode));
        ...
        IDxcBlob* pBlob;
        Error_Check(pResult->GetResult(&pBlob));
        return pBlob;
    }
        ...
        D3D12_DESCRIPTOR_HEAP_TYPE type, bool shaderVisible)
    {
        ...
        Error_Check(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pHeap)));
        return pHeap;
    }
